...
namespace WPFChart3D
{
    class Chart3D
    {
        private float curr_xL = 0;
        private float curr_yL = 0;
        private float curr_zL = 0;
        public Chart3D()
        {
        }
        ...
        {
            if (m_xMax > curr_xL) curr_xL = m_xMax;
            if (m_yMax > curr_yL) curr_yL = m_yMax;
            if (m_zMax > curr_zL) curr_zL = m_zMax;

            m_xAxisLength = curr_xL+500;
            m_yAxisLength = curr_yL+500;
            m_zAxisLength = curr_zL+500;
            m_xAxisCenter = 0;
            m_yAxisCenter = 0;
            m_zAxisCenter = 0;
            m_bUseAxes = true;
        ...
        public void AddAxesMeshes(ArrayList meshs)
        {
            ...
            xAxisCylinder.SetColor(m_axisColorX);
            TransformMatrix.Transform(xAxisCylinder, new Point3D( m_xAxisCenter , m_yAxisCenter, m_zAxisCenter), 1);
            meshs.Add(xAxisCylinder);

            Mesh3D xAxisCone = new Cone3D(2 * radius, 2 * radius, radius * 5, 6);
            xAxisCone.SetColor(m_axisColorX);
            TransformMatrix.Transform(xAxisCone, new Point3D(m_xAxisCenter + m_xAxisLength, m_yAxisCenter, m_zAxisCenter), 1);
            meshs.Add(xAxisCone);
         
            Mesh3D yAxisCylinder = new Cylinder3D(radius, radius, m_yAxisLength, 6);
            yAxisCylinder.SetColor(m_axisColorY);
            TransformMatrix.Transform(yAxisCylinder, new Point3D(m_xAxisCenter , m_yAxisCenter, m_zAxisCenter), 2);
            meshs.Add(yAxisCylinder);
            
            Mesh3D yAxisCone = new Cone3D(2 * radius, 2 * radius, radius * 5, 6);
            yAxisCone.SetColor(m_axisColorY);
            TransformMatrix.Transform(yAxisCone, new Point3D(m_xAxisCenter, m_yAxisCenter + m_yAxisLength, m_zAxisCenter),2);
            meshs.Add(yAxisCone);
   
            Mesh3D zAxisCylinder = new Cylinder3D(radius, radius, m_zAxisLength, 6);
            zAxisCylinder.SetColor(m_axisColorZ);
            TransformMatrix.Transform(zAxisCylinder, new Point3D(m_xAxisCenter , m_yAxisCenter, m_zAxisCenter ), 3);
            meshs.Add(zAxisCylinder);
                         
            Mesh3D zAxisCone = new Cone3D(2 * radius, 2 * radius, radius * 5, 6);
            zAxisCone.SetColor(m_axisColorZ);
            TransformMatrix.Transform(zAxisCone, new Point3D(m_xAxisCenter, m_yAxisCenter, m_zAxisCenter + m_zAxisLength), 3);
            meshs.Add(zAxisCone);
        ...
        public enum SHAPE { BAR, ELLIPSE, CYLINDER, CONE, PYRAMID, CHART };    // shape of the 3d dot in the plot

        protected List<Vertex3D> m_vertices=new List<Vertex3D>();                               // 3d plot data
        ...
        public Color m_axisColorX = Color.FromRgb(0, 50, 200);            // axis color
        public Color m_axisColorY = Color.FromRgb(25, 200, 25);            // axis color

        public Color m_axisColorZ = Color.FromRgb(200, 50, 0);            // axis color
