    ...
    public class Cylinder3D: Mesh3D
    {
        ...
        void SetData(double a, double b, double h)
        {
            ...
            for (int i = 0; i < m_nRes; i++)
            {
                ...
                SetPoint(i, a*(1+System.Math.Cos(aXY)), b*(1+System.Math.Sin(aXY)), 0);
            }

            for (int i = 0; i < m_nRes; i++)
            {
                double aXY = ((double)i) * aXYStep;
                SetPoint(m_nRes + i,  a*(1+System.Math.Cos(aXY)), b*(1+System.Math.Sin(aXY)), h);
            }

            SetPoint(2 * m_nRes, 0, 0, 0);
