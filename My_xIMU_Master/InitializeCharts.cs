        ...
        private void InitTabControl()
        {
            ...
            foreach (xIMU imu in xIMUs)
            {
                ...
                foreach (Chart chart in Charts)
                {
                    ...
                    if (i == 0)
                    {
                        chart.Series[ii].Color = Color.Red;
                    }
                    else if (i == 1)
                    {
                        chart.Series[ii].Color = Color.Green;
                    }
                    chart.Series[ii].ChartType = SeriesChartType.FastLine;

                    if (i >= 9)
                    {
                        name = "Kal. Est. " + chart.Name + imu.ID;
                        chart.Series.Add(new Series(name));
                        if (ii == 0)
                        {
                            chart.Series[ii].Color = Color.Blue;
                        }
                        else if (ii == 1)
                        {
                            chart.Series[ii].Color = Color.Yellow;
                        }
                        chart.Series[ii].ChartType = SeriesChartType.FastLine;
                        /*if (i <= 11)
                        {
